Meteor.methods({
  ...
  adminCheckAdmin: function() {
    ...
    if (this.userId && !Roles.userIsInRole(this.userId, [adminRole()]) && (user.emails.length > 0)) {
      email = user.emails[0].address;
      if (typeof Meteor.settings.adminEmails !== 'undefined') {
        adminEmails = Meteor.settings.adminEmails;
        if (adminEmails.indexOf(email) > -1) {
          console.log('Adding admin user: ' + email);
          return Roles.addUsersToRoles(this.userId, [adminRole()], Roles.GLOBAL_GROUP);
        }
        ...
        return Roles.addUsersToRoles(this.userId, [adminRole()]);
      }
    }
  },
