...
adminCreateTables = function(collections) {
  return _.each(typeof AdminConfig !== "undefined" && AdminConfig !== null ? AdminConfig.collections : void 0, function(collection, name) {
    ...
    if (collection.customColumns && collection.customColumns.length) {
      columns = collection.customColumns;
    } else {
...
Meteor.startup(function() {
  ...
  if (typeof AdminConfig !== "undefined" && AdminConfig !== null && AdminConfig.collections !== null && typeof AdminConfig.usersOptions === 'object') {
    adminEditDelButtons = []
    if (AdminConfig.usersOptions.showEditColumn) {
      adminEditDelButtons.push(adminEditButton);
    }
    if (AdminConfig.usersOptions.showDelColumn) {
      adminEditDelButtons.push(adminDelButton);
    }

    if (AdminConfig.usersOptions.columns) {
      usersColumns = AdminConfig.usersOptions.columns;
    } 
  }
  ...
  return AdminTables.Users = new Tabular.Table({
    ...
    ], usersColumns, adminEditDelButtons),
    dom: adminTablesDom
  });
});