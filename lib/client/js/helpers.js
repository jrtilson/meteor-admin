...
adminCollections = function() {
  ...
  if (typeof AdminConfig !== 'undefined' && typeof AdminConfig.collections === 'object') {
    ...
    if (typeof AdminConfig.usersOptions === 'object') {
      usersOptions = _.extend(usersOptions, AdminConfig.usersOptions);
    }
  }

  collections.Users = usersOptions;
  return _.map(collections, function(obj, key) {
    ...
    if (obj.showNewMenuItem) {
      extend.newPath = Router.path("adminDashboard" + key + "New");
    }

    return obj = _.extend(obj, extend);
  });
};
...
UI.registerHelper('adminRole', function(_id, role) {
  return adminRole();
});
